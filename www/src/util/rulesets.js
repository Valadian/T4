...
let sos_mov = (tp,match_filter=()=>true) => avg(tp.Matches.filter(match_filter).filter(mp=>mp.win!=null).filter((mp) => mp.tournament_opponent_id !== null).map(mp => avg(mp.TournamentOpponent.Matches.filter(match_filter).filter(mp=>mp.win!=null).map(mp => mp['mov']))))
let sos_sos = (tp,match_filter=()=>true) => avg(tp.Matches.filter(match_filter).filter(mp=>mp.win!=null).filter((mp) => mp.tournament_opponent_id !== null).map(mp=>mp.TournamentOpponent.sos))
